...
export default async function MaterialsPage({
  ...
  const sources = await prisma.article.findMany({
    where: { isPublished: true, source: { not: null } },
    select: { source: true },
    distinct: ["source"],
  });
      ...
      <div className="mx-auto max-w-6xl px-4 py-16 sm:px-6 lg:px-8">
        ...
        {sourceList.length > 0 && (
          <div className="mb-8 flex flex-wrap items-center gap-2">
            ...
            <Link
              href={`/materials${category ? `?category=${encodeURIComponent(category)}` : ""}`}
              className={`rounded-full px-4 py-1.5 text-sm font-medium transition-colors ${
                !source
            ...
            {sourceList.map((src) => (
              <Link
                key={src}
                href={`/materials?source=${encodeURIComponent(src)}${category ? `&category=${encodeURIComponent(category)}` : ""}`}
                className={`rounded-full px-4 py-1.5 text-sm font-medium transition-colors ${
                  source === src
                    ? "bg-sky-600 text-white shadow-sm"
                    : "bg-white text-gray-600 ring-1 ring-gray-200 hover:bg-sky-50 hover:text-sky-700"
        ...
        {articles.length === 0 ? (
          ...
          <div className="grid gap-6 sm:grid-cols-2 lg:grid-cols-3">
            {articles.map((article) => {
              ...
              if (isTelegram && article.externalUrl) {
                return (
                  <a
                    key={article.id}
